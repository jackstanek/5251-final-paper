...
to construct a naive code to encode the Latin alphabet, for instance
...
possible to construct a code with an average word length greater than
...
The Lempel-Ziv compression algorithm, commonly referred to as LZ for
short, is one of the most widespread compression schemes in use,
especially when including its large family of derivative methods. It
was first introduced in 1977. \cite{LempelZiv} Algorithms based on
the Lempel-Ziv algorithms are very efficient and work well with a wide
variety of inputs.
